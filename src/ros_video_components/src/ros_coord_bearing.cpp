...
ROSCoordBearing::ROSCoordBearing(QQuickItem *parent) :
    QQuickPaintedItem(parent),
    nh(),
    mapSubscriber(nh, "map", 1),
    tfFilter(mapSubscriber, tfListener, "base_link", 1) {
...
void ROSCoordBearing::mapCallback(const nav_msgs::OccupancyGrid::ConstPtr& data) {
    // map here is not topic, but frame id of target frame
